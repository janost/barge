...
Bridge is a k9s-style TUI for AWS ECS and EC2 management. Go + Bubble Tea + Cobra.
...
config/            Configuration (keybinds, bookmarks, history)
  config.go        Keybinds — YAML at ~/.config/bridge/config.yaml
  bookmarks.go     Named connection targets
  history.go       Recent connections — JSON at ~/.local/share/bridge/
...
- Module path: `github.com/janost/bridge`
- AWS client wrapper package imported as `bridgeaws`
- Exec package imported as `bridgeexec`
