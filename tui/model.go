...
// Model is the root Bubble Tea model for the bridge TUI.
type Model struct {
	...
	clusters   []string
	services   []bridgeaws.ServiceInfo
	tasks      []bridgeaws.TaskInfo
	containers []bridgeaws.ContainerInfo
	instances  []bridgeaws.InstanceInfo
}

// Messages returned by async commands.
type clustersMsg struct{ clusters []string }
type servicesMsg struct{ services []bridgeaws.ServiceInfo }
type tasksMsg struct{ tasks []bridgeaws.TaskInfo }
type instancesMsg struct{ instances []bridgeaws.InstanceInfo }
type errMsg struct{ err error }

// NewModel creates a new TUI model.
// mode: "" = show mode selection, "ecs" = skip to clusters, "ec2" = skip to instances.
func NewModel(client *bridgeaws.Client, command, mode string) Model {
	s := spinner.New()
...
func (m Model) advanceToContainers(containers []bridgeaws.ContainerInfo) (Model, tea.Cmd) {
	if len(containers) == 1 {
...
func (m Model) newContainerList(containers []bridgeaws.ContainerInfo) list.Model {
...
func (m Model) renderBreadcrumb() string {
	...
	if len(parts) == 1 {
		return breadcrumbStyle.Render("bridge")
	}
