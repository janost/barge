...
func (r *ECSTaskResource) FetchCmd(client *bridgeaws.Client) tea.Cmd {
	r.client = client
	return func() tea.Msg {
...
func (r *ECSTaskResource) Actions(row table.Row) []Action {
	...
	for _, t := range r.tasks {
		if t.ID == taskID {
			container, err := bridgeaws.ResolveContainer(t.Containers, "")
			if err != nil {
				return nil
			}
			...
			if r.client != nil {
				...
				actions = append(actions, NewAPIAction(
					"Restart Task",
					func() error {
						return r.client.StopTask(context.Background(), r.cluster, taskIDCopy, "Restarted via bridge")
					},
