...
```bash
go install github.com/janost/barge@latest
```

### Build from repo

```bash
git clone https://github.com/janost/barge.git
cd barge
make build        # builds ./barge
...
| Command | Description |
|---------|-------------|
| `barge tui` | Interactive dashboard (default) |
| `barge exec` | Open shell in ECS task or EC2 instance |
| `barge logs` | Tail CloudWatch logs for a service |
| `barge status` | Service health summary |
| `barge events` | Recent service event timeline |
| `barge cp <src> <dst>` | Copy files to/from a container |
| `barge list` | Tabular resource listing |
| `barge exec save <name>` | Bookmark a connection target |
| `barge exec connect <name>` | Reconnect via bookmark |
| `barge exec bookmarks` | List/manage bookmarks |
| `barge exec history` | Recent connection history |
...
barge exec -c my-cluster -s my-service

# Exec into an EC2 instance by ID
barge exec -i i-0123456789abcdef0

# Tail logs for a service (last 5 minutes, follow)
barge logs -c my-cluster -s my-service --since 5m

# Copy a file from a container
barge cp -c my-cluster -s my-service :/var/log/app.log ./app.log

# Save a bookmark and reconnect later
barge exec save prod-api -c production -s api-service
barge exec connect prod-api
```
...
Config file: `~/.config/barge/config.yaml`
...
Bookmarks are stored in the same config file. History is stored in `~/.local/share/barge/history.json`.
