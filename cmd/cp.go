...
Examples:
  bridge cp ./local-file.txt :/remote/path          # upload
  bridge cp :/remote/path ./local-file.txt           # download
  bridge cp -c cluster -s service :/var/log/app.log . # download, skip TUI
...
func resolveTargetDirect() (cpTarget, error) {
	...
	if cpTask != "" {
		selectedTask, err = bridgeaws.FindTask(tasks, cpTask)
	} else {
	...
	selectedContainer, err := bridgeaws.ResolveContainer(selectedTask.Containers, cpContainer)
	if err != nil {
		return cpTarget{}, err
	}
